...
const PetCardComponent = (pet) => {
  ...
  // Check if other properties exist or provide default values
  ...
  const weight = pet.weight && pet.weight.metric ? `${pet.weight.metric} kg` : 'Unknown Weight';
  const height = pet.height && pet.height.metric ? `${pet.height.metric} cm` : 'Unknown Height';  
